...
class Institute extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, description, levelPrestige, image', 'required'),
			array('levelPrestige', 'numerical', 'integerOnly'=>true),
			array('name, image', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, description, levelPrestige, image', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'tools' => array(self::HAS_MANY, 'Tool', 'institute_id'),
		);
	}
